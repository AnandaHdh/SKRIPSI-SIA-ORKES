...
@include('layouts.partials.header', [
    'judul' => 'Buat Pesanan Baru'
])

<div class="mdk-header-layout__content mdk-header-layout__content--fullbleed mdk-header-layout__content--scrollable page">
	<div class="container-fluid page__container">
		<form method="POST" action="{{ route('member-sewa.store') }}" autocomplete="off">
			@csrf
			...
			<div class="row no-gutters">
				<div class="col-lg-10 card-form__body card-body">
					<div class="form-group">
						...
						<input style="width: 270px;" name="acara" type="text" class="form-control" placeholder="Nama" required="">
					</div>
                    <div class="form-group">
						<label for="opass">Tanggal Acara</label>
						<input style="width: 270px;" name="tgl_acara" type="date" class="form-control" placeholder="Nama" required="">
					</div>
                    <div class="form-group">
						...
						<select name="alamat_id" class="form-control">
                            ...
                            @foreach ($alamat as $place)
                                <option value="{{ $place->id }}">{{ $place->daerah }} - {{ $place->harga->waktu }} - {{ number_format($place->harga->harga, 2, '.' , ',') }}</option>
                            @endforeach
                        </select>
					</div>
				</div>
				...
				<div class="col-lg-2 card-body">
					<a href="{{route('member-sewa.index')}}" class="btn btn-warning">
